...
async function modifySeason(id, field) {
    if (document.getElementById(field).disabled == true)
        ...
        var res = await fetch(`${this.url}season`, {
            ...
            body: JSON.stringify({ id: id, name: document.getElementById(field).value })
        }).then(res => res.json())
        this.response(res)
...
async function modifyArchived(id) {
    ...
    var res = await fetch(`${this.url}seasonarchived`, {
        method: "PUT",
        credentials: "include",
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ id: id, archived: archived })
    }).then(res => res.json())
    this.response(res)
}
